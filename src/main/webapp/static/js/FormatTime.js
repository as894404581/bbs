function formatTime() {
	...
	for(var i = 0; i < time.length; i++) {
		...
		var times = parseInt($(time[i]).html());
		var timeDifference = (new Date().getTime() - times);
		if(timeDifference < 60000) {
			time[i].innerHTML = formats1min(timeDifference);
		} else if(timeDifference < 3600000) {
			time[i].innerHTML = formats60min(timeDifference);
		} else if(timeDifference < 86400000) {
			time[i].innerHTML = formats24hour(timeDifference);

		} else if(timeDifference < 604800000) {
			time[i].innerHTML = formats7day(timeDifference, times);
		} else {
			time[i].innerHTML = format(times, "yyyy-MM-dd")
		}
